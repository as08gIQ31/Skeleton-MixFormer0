...
class Spa_Atten(nn.Module):
    ...
    def forward(self, x): 
        ...
        x1 = x[:,:C//2,:,:]
        x2 = x[:,C//2:C,:,:]
        Q_Spa_Trans = self.avg_pool(x1.permute(0,3,1,2).contiguous())
        K_Spa_Trans = self.avg_pool(x2.permute(0,3,1,2).contiguous())
        ...
        return Spa_atten, Q_Spa_Trans, K_Spa_Trans
...
class Spatial_MixFormer(nn.Module):
    ...
    def forward(self, x0):
        ...
        for i in range(self.num_subset):  
            ...
            m1_2 = self.soft(torch.einsum('nv,nw->nvw', (K1, Q2))).unsqueeze(1).repeat(1,self.out_channels//4,1,1)
            m2_3 = self.soft(torch.einsum('nv,nw->nvw', (K2, Q3))).unsqueeze(1).repeat(1,self.out_channels//4,1,1)
            m3_4 = self.soft(torch.einsum('nv,nw->nvw', (K3, Q4))).unsqueeze(1).repeat(1,self.out_channels//4,1,1)
            m4_1 = self.soft(torch.einsum('nv,nw->nvw', (K4, Q1))).unsqueeze(1).repeat(1,self.out_channels//4,1,1)
            m1 = m1/2 + m1_2/2 
            m2 = m2/2 + m1_2/4 + m2_3/4 
            m3 = m3/2 + m2_3/4 + m3_4/4 
            m4 = m4/2 + m3_4/2 
            atten = torch.cat([m1,m2,m3,m4],dim=1)
            A_final[:,i,:,:,:] = atten  + norm_learn_A[i]
